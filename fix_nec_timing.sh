...
//   nec <8桁hex> [freq_hz]         NEC形式(実測タイミング較正済み、MSBファースト)でエンコードして送信
...
// ---- NECエンコード ----
//
// タイミング定数は教科書的な理想値(9000/4500/562/1687)ではなく、
// 実機からVS1838B経由で実測できた値を使う。受信回路自身の応答遅延で
// 多少歪む(mark側がやや長く、space側がやや短く出る)ため、
// 「理論値」より「同じ受信回路で動作実績のある実測値」の方が再現性が高い。
#define NEC_LEADER_MARK  8820
#define NEC_LEADER_SPACE 4365
#define NEC_BIT_MARK      572
#define NEC_SPACE_0       532
#define NEC_SPACE_1      1636
...
static size_t nec_encode(uint32_t bytes32, bool lsb_first_per_byte, bool reverse_byte_order,
    ...
    size_t n = 0;
    if (n < out_max) out[n++] = NEC_LEADER_MARK;
    if (n < out_max) out[n++] = NEC_LEADER_SPACE;
    ...
    for (int byte_i = 0; byte_i < 4; byte_i++) {
        ...
        for (int bit_i = 0; bit_i < 8; bit_i++) {
            ...
            if (n < out_max) out[n++] = NEC_BIT_MARK;
            if (n < out_max) out[n++] = bit ? NEC_SPACE_1 : NEC_SPACE_0;
        }
    }
    if (n < out_max) out[n++] = NEC_BIT_MARK;
    return n;
...
static void cmd_nec(const char *args) {
    char hex[32];
    const char *sp = strchr(args, ' ');
    uint32_t freq = 38000;

    if (sp) {
        size_t hlen = (size_t) (sp - args);
        if (hlen >= sizeof(hex)) hlen = sizeof(hex) - 1;
        memcpy(hex, args, hlen);
        hex[hlen] = '\0';
        freq = (uint32_t) strtoul(sp + 1, NULL, 10);
        if (freq == 0) freq = 38000;
    } else {
        strncpy(hex, args, sizeof(hex) - 1);
        hex[sizeof(hex) - 1] = '\0';
    }

    uint32_t v = (uint32_t) strtoul(hex, NULL, 16);
    // rx時のnec_try_decodeは「最初に届いたbitをMSBとして32bit値に詰める」方式で
    // addr/cmdを計算している。それを忠実に再現するには、ここも
    // MSBファースト・byte順そのまま（lsb_first_per_byte=false, reverse_byte_order=false）
    // が対称な組み合わせになる。
    size_t n = nec_encode(v, false, false, g_pattern, PATTERN_MAX);
    printf("NEC(MSBファースト、実測タイミング較正済み) freq=%luHz を送信: 0x%08lX (entries=%u)\r\n",
           (unsigned long) freq, (unsigned long) v, (unsigned) n);
    ir_tx_send(g_pattern, n, freq);
    printf("送信完了\r\n");
}

static void cmd_necvariants(const char *args) {
    char hex[32];
    const char *sp = strchr(args, ' ');
    uint32_t freq = 38000;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        printf("[%d/4] %s freq=%luHz を送信 (entries=%u)\r\n", i + 1, variants[i].label,
               (unsigned long) freq, (unsigned) n);
        ir_tx_send(g_pattern, n, freq);
        sleep_ms(2000);
    }
...
static void cmd_help(void) {
    ...
    printf("  nec <8桁hex> [freq_hz]             NEC形式(実測タイミング較正済み)で送信\r\n");
